...
public class NegoStrat {
	...
	private double MIN_SELF_KALAI_UTILITY = 0.8;
	...
	public BidDetails kalaiOptimalBid(double alpha){
		...
		while (it.hasNext()) {
			...
			for (BidDetails bid:allbids){
				...
				while (inneriterator.hasNext()) {
					...
					product = product * ((OpponentModel)innerpair.getValue()).getOpponentUtility(bid.getBid());
				}
		...
		double nashcalc = sessionInfo.getUtilitySpace().getUtility(nashBid.getBid());
		double kalaicalc = sessionInfo.getUtilitySpace().getUtility(bidFinal.getBid());
		...
		{
			System.out.println("NASH > Kalai, returning NASH!");
			return nashBid;

		}
		if(bidFinal!=null)
		{
			System.out.println("Returning Kalai Bid!");
		}
		else
		{
			System.out.println("BID FINAL = NULL");
			return randomBid();
		}
